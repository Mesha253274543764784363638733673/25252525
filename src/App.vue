...
const fetchFavorites = async () => {
  try {
    ...
    const { data: favorites } = await axios.get(`https://d4b682c8f9a42a9e.mokky.dev/favorites`)
...
const addToFavorite = async (item) => {
  try {
    if (!item.isFavorite) {
      ...
      const { data } = await axios.post(`https://d4b682c8f9a42a9e.mokky.dev/favorites`, obj) // Отправляем POST-запрос для добавления элемента в избранное
      ...
      await axios.delete(`https://d4b682c8f9a42a9e.mokky.dev/favorites/${item.favoriteId}`) // Отправляем DELETE-запрос для удаления элемента из избранного
...
const fetchItems = async () => {
  try {
    ...
    const { data } = await axios.get(`https://d4b682c8f9a42a9e.mokky.dev/items`, { params })
